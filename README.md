...
- `/aiimg帮助`：查看用法说明。
- `/设置ai配置 <gemini|geminichat|dalle|gpt> <api_base> <api_key> <model_name>`：在群聊或私聊中设置个人模型类型与 API，模型名必填。
- `/查看ai配置`：在群聊或私聊中查看自己的个人配置；群聊未设置时提示使用插件默认配置。

个人配置按用户保存，在群聊和私聊中共用，仅对设置者本人生效。群聊生图/改图优先使用发送者的个人配置，未设置时使用 AstrBot 插件配置中的默认类型、API 地址、密钥和模型。私聊仍需先设置个人配置。群聊使用个人配置时仍遵守群白/黑名单和群限流规则。

生图/改图成功或失败时，提示会注明本次使用的是“个人配置”还是“公共配置”（插件后台默认配置）。
